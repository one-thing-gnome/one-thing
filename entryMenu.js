...
var EntryMenu = class extends PopupMenu.PopupMenu {
  ...
  _onCopyActivated() {
    ...
    if (selection.length === 0) selection = this._entry.clutter_text.get_text();
...
function _setMenuAlignment(entry, stageX) {
  ...
  if (success) entry.menu.setSourceAlignment(entryX / entry.width);
}
...
function _onPopup(actor, entry) {
  ...
  if (success) entry.menu.setSourceAlignment(textX / entry.width);
  entry.menu.open(BoxPointer.PopupAnimation.FULL);
}
